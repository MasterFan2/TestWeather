...
public class NetworkUtil {
     ...
     * @return
     */
    private static NetInfo.NetType getNetworkType(Context context) {

        NetInfo.NetType netType = NetInfo.NetType.T_UNKNOWN;
        ...
        if (networkInfo != null && networkInfo.isConnected()) {
            ...
            if (type.equalsIgnoreCase("WIFI")) {
                netType = NetInfo.NetType.T_WIFI;
            } else if (type.equalsIgnoreCase("MOBILE")) {

                if(networkInfo.getSubtype() == TelephonyManager.NETWORK_TYPE_LTE){
                    netType = NetInfo.NetType.T_4G;
                }else {

                    String proxyHost = android.net.Proxy.getDefaultHost();

                    netType = TextUtils.isEmpty(proxyHost) ?
                            (isFastMobileNetwork(context) ? NetInfo.NetType.T_3G  : NetInfo.NetType.T_2G)
                            : NetInfo.NetType.T_WAP;
                }
            }
            //else {T_UNKNOW}
        } else {
            netType = NetInfo.NetType.T_NO_CONN;
        }
    ...
    public static NetInfo getNetworkInfo(Context context){
        ...
        NetInfo.NetType netWorkType = getNetworkType(context);
